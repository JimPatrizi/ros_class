    ...
    std::vector<Eigen::Vector3d> reachable, approachable;
        ...
        for (double x_des = x_min;x_des<x_max;x_des+=dx) {
            for (double y_des = y_min; y_des<y_max; y_des+=dy) {
                ...
                nsolns = UR10IkSolver.ik_solve_approx_wrt_torso(a_tool_des, q_solns);
                //valid = UR10IkSolver_.improve_7dof_soln_wrt_torso(des_flange_affine, q_in, q_7dof_precise);
